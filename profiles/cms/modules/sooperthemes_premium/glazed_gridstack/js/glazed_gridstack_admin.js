(function ($) {
  Drupal.behaviors.initGridstackAdmin = {
    attach: function (context, settings) {
      var adminFormSelector = '#glazed-gridstack-grid-stack-admin';
      if ($(document).find(adminFormSelector + ' .grid-stack').length <= 0){
        return false;
      }
      var layoutData = settings.glazedMagazine.layoutDataAdmin;
      ...
      new function () {
        ...
        this.save_grid = function () {
          ...
          $(adminFormSelector + ' #edit-style-options-gridstack-layout-data').val(JSON.stringify(this.serialized_data));
        }.bind(this);

        $(adminFormSelector + ' .grid-stack').on('change', function (e, items) {
          this.save_grid();
        }.bind(this));
        $(adminFormSelector + ' #edit-style-options-gridstack-items').on('input', function (e) {
          var numValue = $(adminFormSelector + ' #edit-style-options-gridstack-items').val();
          $(adminFormSelector + ' #edit-style-options-gridstack-layout').val('custom').attr("selected", "selected");
          if (!numValue || numValue > 100) {
            numValue = 1;
          }
          this.create_grid(numValue);
        }.bind(this));
        $(adminFormSelector + ' #edit-style-options-gridstack-layout').on('change', function (e) {
          var layoutValue = $(adminFormSelector + ' #edit-style-options-gridstack-layout').val();
          ...
          if (layoutValue && layoutValue != 'custom' && exampleLayouts[layoutValue]) {
            $(adminFormSelector + ' #edit-style-options-gridstack-items').val(exampleLayouts[layoutValue].length);
