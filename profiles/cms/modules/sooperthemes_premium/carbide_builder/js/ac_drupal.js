...
(function ($) {
  ...
  function carbide_builder_select(options, input) {
  ...
  window.images_select = function (input, delimiter) {
    ...
    $(input).closest('.ac-select-image').once('mediaBrowserLaunch', function () {
      ...
      var image_styles_select = carbide_builder_select(Drupal.settings.carbide_builder.image_styles, image_styles_input);
      // Get image style url.
      function make_image_src(src) {
        ...
        if (q.length > 1) {
          ...
          if ($(image_styles_input).val() != '') {
            $.ajax({
              type: 'POST',
              url: Drupal.settings.basePath + '?q=carbide_builder_image_style_url',
              data: {
  ...
  window.carbide_ckeditor = function (textarea) {
  ...
  window.carbide_t = function (text) {
    return Drupal.t(text);
  };
  window.carbide_editable = ['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'img:not(.not-editable)', 'a:not(.not-editable)', 'i:not(.not-editable)'];
  window.carbide_styleable = [];
  window.carbide_textareas = [];
  window.carbide_formats = [];
  Drupal.behaviors.carbide_builder = {
    attach: function (context, settings) {
      if (fp + 'modal' in $.fn && fp + 'popover' in $.fn) {
        for (var i = 0; i < window.carbide_textareas.length; i++) {
          (function (textarea, format) {
            $('#' + format).change(function () {
              var $textarea  = $('#' + textarea);
              if ($(this).val() == 'carbide_builder') {
                window.carbide_editor = true;
                $textarea.carbide_builder();
                if (Drupal.hasOwnProperty('wysiwygAttach'))
                  Drupal.wysiwygAttach(context, textarea);
                $textarea.hide();
              }
              else {
                $textarea.carbide_builder('hide');
                $textarea.show();
              }
            });
            _.defer(function () {
              if (_.isUndefined($('#' + textarea).data('carbide_builder'))) {
                $('#' + format).trigger('change');
              }
            });
          })(window.carbide_textareas[i], window.carbide_formats[i]);
        }
      }
    }
  };
